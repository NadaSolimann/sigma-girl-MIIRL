...
def order_by_fastest_to_goal(trajs, t1, t2, goal):
    trajs = [(t1, len(trajs[t1])), (t2, len(trajs[t2]))]
    trajs.sort(key=lambda x: x[1] if x[0][-1] == goal else float('inf'))
    preferred_traj = trajs[0][0] if trajs[0][0][-1] == goal else min(trajs, key=lambda x: x[1])[0]
    return (preferred_traj, t1) if preferred_traj == t1 else (preferred_traj, t2)


def get_traj_preferences(trajs, goal):
    prefs = []
    trajs_at_goal, trajs_not_at_goal = [], []
    for t1 in range(len(trajs)):
        x1, y1 = trajs[t1][-1][0], trajs[t1][-1][1]
        if (int(x1) == goal[0] and int(y1) == goal[1]):
            trajs_at_goal.append(t1)
        else:
            trajs_not_at_goal.append(t1)

    for t1 in trajs_at_goal:
        t2 = random.choice(trajs_not_at_goal)
        prefs.append((t1, t2))

    return prefs


def get_trajs_lens(trajs):
    lens = []
    for traj in trajs:
...
def run(id, seed, args):
    ...
    for i_beta, beta in enumerate(betas):
        ...
        for n_i, n_sample in enumerate(n_samples_irl):
            ...
            for s2 in samp:
                ...
                agent_to_data['down' + str(s)] = [
                    path + "down/dataset_%s/" % (
                        str(s2 + 0)), []]
            gt_intents = [0]*n_sample + [1]*n_sample*2
            all_states = []
            all_actions = []
            ...
            preferences = get_traj_preferences(all_states, goal)
            len_trajs = get_trajs_lens(all_states)
